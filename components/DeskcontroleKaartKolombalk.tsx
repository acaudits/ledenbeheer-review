...
type KolomMenuProps = {
  kolom: Kolom;
  filter: string;
  sortering: DeskcontroleCardSortering | undefined;
...
function KolomMenu({
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      ...
      try {
        ...
        if (zoekwaarde.trim() && kolom.type !== "datum") {
  ...
  }, [filter, kolom.sleutel, kolom.type, menuGeopend, zoekwaarde]);
  ...
  return (
    <details
      ...
      data-deskcontrole-kolommenu="true"
      onToggle={(event) => {
        ...
        document
          .querySelectorAll<HTMLDetailsElement>(
            'details[data-deskcontrole-kolommenu="true"][open]',
          )
          .forEach((menu) => {
            if (menu !== geopendMenu) {
              menu.open = false;
...
export function DeskcontroleKaartKolombalk({
  kolommen,
  filters,
  sorteringen,
  onFilterWijzigen,
