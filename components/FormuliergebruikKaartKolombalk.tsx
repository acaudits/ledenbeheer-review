...
export type FormuliergebruikCardSortering = {
  sleutel: string;
...
type KolomMenuProps = {
  kolom: Kolom;
  filter: string;
  sortering: FormuliergebruikCardSortering | undefined;
...
function KolomMenu({
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      ...
      try {
        ...
        if (zoekwaarde.trim() && !DATUMKOLOMMEN.has(kolom.sleutel)) {
  ...
  return (
    <details
      ...
      data-formuliergebruik-kolommenu="true"
      onToggle={(event) => {
        ...
        document
          .querySelectorAll<HTMLDetailsElement>(
            'details[data-formuliergebruik-kolommenu="true"][open]',
          )
          .forEach((menu) => {
            if (menu !== geopendMenu) {
              menu.open = false;
      ...
      <div className="absolute left-0 right-0 top-full z-[120] mt-1 flex max-h-[min(28rem,calc(100vh-1rem))] min-w-60 flex-col overflow-hidden rounded-xl border border-slate-200 bg-white shadow-2xl ring-1 ring-slate-900/5">
        ...
        <div className="min-h-0 flex-1 overflow-y-auto p-2.5">
          ...
          <div className="mt-1.5 max-h-48 overflow-y-auto rounded-lg border border-slate-200 bg-white">
            ...
            ) : DATUMKOLOMMEN.has(kolom.sleutel) ? (
...
export function FormuliergebruikKaartKolombalk({
  kolommen,
  filters,
  sorteringen,
  onFilterWijzigen,
