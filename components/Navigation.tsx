...
const navigatie = [
  ...
  {
    naam: "Non-conformiteiten",
    href: "/non-conformiteiten",
    icoon: (
      <svg viewBox="0 0 24 24" fill="none" className="size-5">
        <path
          d="M12 3 3.5 19h17L12 3Zm0 5v5m0 3.5v.5"
          stroke="currentColor"
          strokeWidth="1.8"
          strokeLinecap="round"
          strokeLinejoin="round"
...
function MenuInhoud({ pathname, rollen, sluitMenu }: MenuInhoudProps) {
  return (
    <>
      <div className="border-b border-white/10 px-5 py-6">
        ...
        <GebruikersAanwezigheid rollen={rollen} pathname={pathname} />
      </div>
        ...
        {navigatie
          ...
          .map((item) => {
            ...
            return (
              ...
              >
                <span
                  className={actief ? "text-emerald-400" : "text-slate-500"}
                >
                  {item.icoon}
                </span>
      ...
      <div className="space-y-3 border-t border-white/10 p-4">
        <MeldingenLink sluitMenu={sluitMenu} />

        <ProfielLink sluitMenu={sluitMenu} />

        <BeheerderLink rollen={rollen} sluitMenu={sluitMenu} />
  ...
  useEffect(() => {
    ...
    async function laadRol() {
      try {
        ...
        if (actief && Array.isArray(gegevens.rollen)) {
          setRollen(
            gegevens.rollen.filter(
              (rol: unknown): rol is string => typeof rol === "string",
            ),
          );
        }
      } catch (fout) {
        console.error("Gebruikersrollen ophalen mislukt:", fout);
      }
    }
  ...
  return (
    <>
      ...
      <aside className="fixed inset-y-0 left-0 z-40 hidden w-72 flex-col overflow-y-auto border-r border-white/5 bg-[#071512] lg:flex">
        <MenuInhoud pathname={pathname} rollen={rollen} />
      </aside>
