...
export type NaFinalisatieCardSortering = {
  sleutel: string;
...
type Props = {
  ...
  sorteringen: NaFinalisatieCardSortering[];
...
type KolomMenuProps = {
  kolom: Kolom;
  filter: string;
  sortering: NaFinalisatieCardSortering | undefined;
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      ...
      try {
        ...
        if (zoekwaarde.trim() && kolom.sleutel !== "datumNaFinalisatie") {
      ...
      <div className="absolute left-0 right-0 top-full z-[120] mt-1 flex max-h-[min(28rem,calc(100vh-1rem))] min-w-60 flex-col overflow-hidden rounded-xl border border-slate-200 bg-white shadow-2xl ring-1 ring-slate-900/5">
        ...
        <div className="min-h-0 flex-1 overflow-y-auto p-2.5">
          ...
          <div className="mt-1.5 max-h-48 overflow-y-auto rounded-lg border border-slate-200 bg-white">
            ...
            ) : kolom.sleutel === "datumNaFinalisatie" ? (
...
export function NaFinalisatieKaartKolombalk({
  kolommen,
  filters,
  sorteringen,
  onFilterWijzigen,
