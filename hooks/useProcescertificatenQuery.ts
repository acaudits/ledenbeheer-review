...
export type ProcescertificaatCardSortering = {
  sleutel: string;
...
type ProcescertificatenQueryInvoer = {
  ...
  sorteringen: ProcescertificaatCardSortering[];
};
...
export const PROCESCERTIFICATEN_QUERY_SLEUTEL = ["procescertificaten"] as const;
...
function isPagina(waarde: unknown): waarde is ProcescertificatenPagina {
...
export function useProcescertificatenQuery({
  ...
  const query = useInfiniteQuery({
    ...
    queryKey: [...PROCESCERTIFICATEN_QUERY_SLEUTEL, aanvraagSleutel],
        ...
        parameters.set(
          ...
          aanvraagSleutel.sorteringen
            .map(
              (criterium) =>
                `${criterium.sleutel}:${
                  criterium.richting === "oplopend" ? "asc" : "desc"
                }`,
            )
            .join(","),
        );
