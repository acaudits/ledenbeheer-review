...
export const DESKCONTROLES_QUERY_SLEUTEL =
  ["deskcontroles"] as const;
...
export const DESKCONTROLE_SERVERGEGEVENS_EVENT =
  "deskcontrole-servergegevens";
...
export function useDeskcontrolesQuery({
  ...
  const [uitgesteld, setUitgesteld] =
    useState({
      zoekterm,
      filters,
      datumFilters,
  ...
  const aanvraag = useMemo(
    () => ({
      zoekterm:
        uitgesteld.zoekterm.trim(),
      filters: uitgesteld.filters,
      datumFilters:
        uitgesteld.datumFilters,
      sortering,
      dashboardFilter,
    }),
    [
  ...
  const query = useInfiniteQuery({
    ...
    queryFn: async ({
      ...
      if (aanvraag.dashboardFilter) {
        parameters.set(
          "dashboardFilter",
          aanvraag.dashboardFilter,
        );
      }

      for (
        const [sleutel, parameter]
        of Object.entries(
      ...
      for (
        ...
        of Object.entries(
          aanvraag.datumFilters,
        )
      ) {
        const suffix =
          `${sleutel[0].toUpperCase()}${sleutel.slice(1)}`;
      ...
      if (!antwoord.ok) {
        throw new Error(
          await leesFoutmelding(antwoord),
        );
      }
  ...
  const rijen = useMemo(() => {
    const perId =
      new Map<number, DeskcontroleRij>();
