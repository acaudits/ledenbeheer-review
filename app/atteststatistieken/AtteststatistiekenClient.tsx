...
import AtteststatistiekenImport from "./AtteststatistiekenImport";
...
function Melding({ status }: { status: CorrectieStatus }) {
  if (!status.message) {
    return null;
  }
...
export default function AtteststatistiekenClient({
  ...
  const [correctieStatus, correctieActie, correctieBezig] = useActionState(
    voegAttestCorrectieToe,
    {},
  );

  const [achtergrondBezig, startAchtergrondActie] = useTransition();

  const [persoonZoeken, setPersoonZoeken] = useState("");

  const [bedrijfZoeken, setBedrijfZoeken] = useState("");

  const [correctieZoeken, setCorrectieZoeken] = useState("");

  const [actieMelding, setActieMelding] = useState<CorrectieStatus>({});
  ...
  const gefilterdePersonen = useMemo(() => {
    const zoekterm = persoonZoeken.trim().toLocaleLowerCase("nl-BE");
  ...
  const gefilterdeBedrijven = useMemo(() => {
    const zoekterm = bedrijfZoeken.trim().toLocaleLowerCase("nl-BE");
    ...
    return bedrijven.filter((bedrijf) =>
      bedrijf.bedrijfsnaam.toLocaleLowerCase("nl-BE").includes(zoekterm),
    );
  }, [bedrijven, bedrijfZoeken]);

  const gefilterdeCorrecties = useMemo(() => {
    const zoekterm = correctieZoeken.trim().toLocaleLowerCase("nl-BE");
  ...
  function correctiesToepassen() {
    ...
    startAchtergrondActie(async () => {
      const resultaat = await pasAttestCorrectiesToe();
  ...
  function correctieVerwijderen(id: number) {
    ...
    startAchtergrondActie(async () => {
      const resultaat = await verwijderAttestCorrectie(id);
    ...
    <div className="space-y-6">
      <section className={`${BEHEER_TABEL_STIJLEN.kader} p-5`}>
        ...
        <p className="mt-1 text-sm text-slate-600">
          De import vervangt lijst 1 en lijst 2 volledig. Lijst 3 blijft
          behouden.
        </p>

        <AtteststatistiekenImport />

        {laatsteImport ? (
          <dl className="mt-4 grid gap-3 rounded-xl bg-slate-50 p-4 text-sm sm:grid-cols-2 lg:grid-cols-4">
            <div>
              <dt className="font-semibold text-slate-500">Bestand</dt>
            ...
            <div>
              <dt className="font-semibold text-slate-500">Geïmporteerd</dt>
              <dd className="mt-1 text-slate-900">
            ...
            <div>
              <dt className="font-semibold text-slate-500">Excelrijen</dt>
              <dd className="mt-1 text-slate-900">
            ...
            <div>
              ...
              <dd className="mt-1 text-slate-900">
                {datumTijd(laatsteImport.correctiesToegepastOp)}
              </dd>
            </div>
          </dl>
        ) : (
        ...
        <section className={BEHEER_TABEL_STIJLEN.kader}>
          <div className="border-b border-slate-200 p-5">
            ...
            <input
              type="search"
              value={persoonZoeken}
              onChange={(event) => setPersoonZoeken(event.target.value)}
          ...
          <div className="max-h-[600px] overflow-auto">
            <table className={`${BEHEER_TABEL_STIJLEN.tabel} text-sm`}>
              <thead className={`${BEHEER_TABEL_STIJLEN.kop} sticky top-0`}>
                <tr>
                  <th className="px-4 py-3">PersoonsID</th>
                  <th className="px-4 py-3">Naam</th>
                  <th className="px-4 py-3 text-right">Aantal attesten</th>
                </tr>
              </thead>
              ...
              <tbody className="divide-y divide-slate-100">
                {gefilterdePersonen.map((persoon) => (
                  <tr key={persoon.id}>
                    ...
                    <td className="px-4 py-3 text-slate-700">{persoon.naam}</td>
        ...
        <section className={BEHEER_TABEL_STIJLEN.kader}>
          <div className="border-b border-slate-200 p-5">
            ...
            <input
              type="search"
              value={bedrijfZoeken}
              onChange={(event) => setBedrijfZoeken(event.target.value)}
      ...
      <section className={`${BEHEER_TABEL_STIJLEN.kader} p-5`}>
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          ...
          <button
            ...
            disabled={
              ...
              correcties.length === 0 ||
              !laatsteImport ||
              Boolean(laatsteImport.correctiesToegepastOp)
            }
        ...
        >
          ...
          <div className="flex items-end">
            ...
            >
              {correctieBezig ? "Toevoegen..." : "Correctie toevoegen"}
            </button>
          </div>
        </form>
        ...
        <input
          ...
          onChange={(event) => setCorrectieZoeken(event.target.value)}
            ...
            <tbody className="divide-y divide-slate-100">
              {gefilterdeCorrecties.map((correctie) => (
                <tr key={correctie.id}>
                  ...
                  <td className="px-4 py-3 text-slate-700">{correctie.naam}</td>
                  ...
                  <td className="px-4 py-3 text-right">
                    <button
                      ...
                      onClick={() => correctieVerwijderen(correctie.id)}
