...
export async function importeerIngeplandeTerreincontroleStatussen(
  _vorigeStatus:
    StatusExcelImportState,
  formData: FormData,
): Promise<StatusExcelImportState> {
  await vereisMachtiging("TERREINCONTROLES_STATUS_IMPORTEREN");
  ...
  if (
    ...
    return {
      ...
      errors: {
        excelBestand:
          'Kolom A moet "inputId" bevatten en kolom B moet "status" bevatten.',
      },
    };
  }
  ...
  const terreincontrolePerAttestId =
    new Map(
      bestaandeTerreincontroles.flatMap(
        (terreincontrole) => {
          const attestId =
  ...
  for (
    ...
    const terreincontrole =
      terreincontrolePerAttestId.get(
        statusRij.attestId,
      );
