...
export type TerreincontroleBulkStatus =
  "GEIMPORTEERD" | "OVERGESLAGEN" | "MISLUKT";
...
function leesCelTekst(cel: ExcelJS.Cell) {
  ...
  if (typeof waarde === "object" && "text" in waarde) {
    return String(waarde.text ?? "").trim();
  }

  if (typeof waarde === "object" && "result" in waarde) {
    return String(waarde.result ?? "").trim();
  }
  ...
  return String(cel.text ?? "").trim();
}

async function controleerBestand(bestand: File): Promise<Voorcontrole> {
  ...
  if (!werkblad) {
    return {
      succes: false,
      attestnummer: "",
      message: `Het werkblad "${WERKBLAD_NAAM}" ontbreekt.`,
    };
  }
  ...
  for (let rij = 16; rij <= werkblad.rowCount; rij++) {
    if (leesCelTekst(werkblad.getCell(`B${rij}`))) {
...
function geefImportfout(status: TerreincontroleImportState) {
  return (
    status.errors?.excelBestand || status.message || "Onbekende importfout."
  );
}

function isDuplicaatmelding(message: string) {
  const waarde = message.toLocaleLowerCase("nl-BE");

  return (
    waarde.includes("bestaat al") ||
    waarde.includes("al gemporteerd") ||
    waarde.includes("mogelijk al")
  );
}
...
export async function importeerTerreincontroleBatch(
  ...
  if (bestanden.length === 0) {
    return {
      succes: false,
      message: "Deze batch bevat geen terreincontrolebestanden.",
      resultaten: [],
    };
  }
  ...
  const resultaten: TerreincontroleBulkResultaat[] = [];

  for (const bestand of bestanden) {
    const voorcontrole = await controleerBestand(bestand);
    ...
    let status: TerreincontroleImportState | null = null;

    let geimporteerd = false;

    try {
      status = await importeerTerreincontroleUitExcel({}, formulier);
    } catch (error) {
    ...
    if (geimporteerd) {
      const aangemaakt = await prisma.terreincontroleDossier.findFirst({
        where: {
          attestnummer,
        },
        select: {
      ...
      resultaten.push({
        ...
        attestnummer,
        status: "GEIMPORTEERD",
        message: "Terreincontrole en vaststellingen geïmporteerd.",
        terreincontroleId: aangemaakt?.id,
        aantalVaststellingen:
